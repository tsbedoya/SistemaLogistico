...
export default function ClientesPage() {
  ...
  async function handleGuardar(e) {
    ...
    try {
      if (editando) {
        await clienteService.update(editando.id, form)
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full bg-white text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {clientes.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-gray-400">No hay clientes</td>
              </tr>
            ) : (
              clientes.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      {esAdmin && <button onClick={() => handleEliminar(c.id)} className="text-red-500 hover:underline text-xs">Eliminar</button>}
                    </div>
                  </td>
                </tr>
              ))
