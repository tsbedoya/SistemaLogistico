...
import { envioTerrestreService } from '../services/enviosService'
...
function EnvioTerrestreForm({ initial, onGuardar, onCancelar }) {
  ...
  const precioFinal = form.precio_base && form.cantidad_producto
    ? Number(form.cantidad_producto) > 10
      ? (Number(form.precio_base) * 0.95).toFixed(2)
  ...
  v => v.bodega_id === Number(form.bodega_id)
)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-sm font-medium text-gray-700">Bodega de entrega</label>
        ...
        <div>
          ...
          <select required value={form.vehiculo_id} onChange={(e) => set('vehiculo_id', e.target.value)} className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white">
            <option value="">Selecciona...</option>
      ...
      {precioFinal && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
          ...
          {Number(form.cantidad_producto) > 10 && (
            <span className="text-green-600 ml-2">(5% de descuento)</span>
          )}
        </div>
      )}
...
const emptyForm = {
  cliente_id: '', producto_id: '', bodega_id: '', vehiculo_id: '',
...
export default function EnviosTerrestresPage() {
  ...
  async function cargarEnvios() {
    try {
      setLoading(true)
      const res = await envioTerrestreService.getAll()
  ...
  async function handleGuardar(data) {
    ...
    try {
      if (editando) {
        await envioTerrestreService.update(editando.id, data)
        toast.success('Envío actualizado')
      } else {
        await envioTerrestreService.create(data)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Envíos Terrestres</h1>
        <button
          onClick={abrirCrear}
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full bg-white text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-gray-600">N. Guía</th>
              <th className="px-4 py-3 text-left font-semibold text-gray-600">Cliente</th>
              <th className="px-4 py-3 text-left font-semibold text-gray-600">T. Producto</th>
              ...
              <th className="px-4 py-3 text-left font-semibold text-gray-600">Precio envio</th>
              <th className="px-4 py-3 text-left font-semibold text-gray-600">F. registro</th>
              <th className="px-4 py-3 text-left font-semibold text-gray-600">F. entrega</th>
            ...
            {envios.length === 0 ? (
              ...
              envios.map((e) => (
                <tr key={e.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-3">{e.bodega?.nombre}</td>
                  <td className="px-3 py-3">{e.vehiculo?.placa}</td>
      ...
      {showModal && (
        <Modal
          title={editando ? 'Editar Envío Terrestre' : 'Nuevo Envío Terrestre'}
